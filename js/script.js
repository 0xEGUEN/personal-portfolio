...
const isLowEndDevice = () => {
  ...
  const isTouchDevice = isMobileDevice() || navigator.maxTouchPoints > 0;
  const hasSlowConnection = Boolean(connection && (connection.saveData || ['slow-2g', '2g', '3g'].includes(connection.effectiveType)));
  const lowMemory = deviceMemory !== null && deviceMemory <= 4;
  const lowCores = hardwareConcurrency !== null && hardwareConcurrency <= 4;
  return Boolean(hasSlowConnection || (isTouchDevice && (lowMemory || lowCores)));
};
const IS_LOW_END_DEVICE = isLowEndDevice();
window.__SITE_LOW_END_DEVICE__ = IS_LOW_END_DEVICE;
document.documentElement.classList.toggle('low-end-mode', IS_LOW_END_DEVICE);
...
const shouldUseDesktopEffects = () => {
  ...
  if (IS_LOW_END_DEVICE) return false;
